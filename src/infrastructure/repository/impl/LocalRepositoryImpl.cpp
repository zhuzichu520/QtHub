...
LocalRepositoryImpl::LocalRepositoryImpl(QObject* parent) : LocalRepository{parent} {
    qRegisterMetaType<HistoryPo*>();
    qRegisterMetaType<DB*>();
    db.setDriver("QSQLITE");
    db.setDatabaseName(AppConfig::instance()->getDatabseDir()+QDir::separator()+"data.sqlite");
